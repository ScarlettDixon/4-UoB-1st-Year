...
class Task_3:
    ...
    def search_store(dictcheck, diction):
        for i in dictcheck.keys():
            for j in diction.keys():
                if i == j and dictcheck[i] != diction[j]:
                    print ("Collision found In storage")
                    HashedInpOne = hashlib.sha1(dictcheck[i].encode("ascii")).hexdigest()
                    HashedInpTwo = hashlib.sha1(diction[j].encode("ascii")).hexdigest()
                    HashedStore = HashedInpOne[0:8]
                    print("Plaintext one is:", dictcheck[i])
                    print("Plaintext two is:", diction[j])
                    print("The full hash for the first plaintext is:",HashedInpOne)
                    print ("The full hash for the first plaintext is:", HashedInpTwo)
                    print ("The part of the hash shared is:", HashedStore)
                    return False
        return True
    ...
    def implement():
        ...
        while run == True :
            ...
            if (Tries % 1000000) == 0:
                if (stored % 2) == 0:
                    print("Storing")
                    with open(('S1-Task3-Dumps/S1-Task3-store-'+ str(stored) +'.dump'), 'wb') as file:
                            pickle.dump(diction, file, protocol=pickle.HIGHEST_PROTOCOL) # use `pickle.loads` to do the reverse
                    diction = {}
                    stored +=1
                    Tries = 0
                else:
                    print("Loading")
                    with open(('S1-Task3-Dumps/S1-Task3-store-'+ str(stored - 1) +'.dump'), 'rb') as file:
                            dictcheck = pickle.load(file)
                    #print(diction)
                    #print(dictcheck)
                    run = Task_3.search_store(dictcheck, diction)
                    with open(('S1-Task3-Dumps/S1-Task3-store-'+ str(stored) +'.dump'), 'wb') as file:
                            pickle.dump(diction, file, protocol=pickle.HIGHEST_PROTOCOL) # use `pickle.loads` to do the reverse
                    diction = {}
                    stored +=1
